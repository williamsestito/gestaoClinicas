...
function toggleStatus(patient: PatientRow) {
    const action = patient.status === 'active' ? deactivate : activate;

    router.patch(action(patient.id).url, {}, { preserveScroll: true });
}
...
function restorePatient(patient: PatientRow) {
    router.post(restore(patient.id).url, {}, { preserveScroll: true });
}
</script>
...
<template>
    ...
    <div class="flex flex-col space-y-6 p-4">
        ...
        >
            <template #actions>
                <Button as-child>
                    <Link :href="create().url">Novo paciente</Link>
                </Button>
            </template>
        </PageHeader>
