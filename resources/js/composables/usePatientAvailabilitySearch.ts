...
export type AvailabilityProfessional = { id: string; name: string };
...
/**
 * Mesma cadeia unidade → especialidade → serviço → profissional (opcional)
 * → data → horário de usePublicAvailabilitySearch(), mas contra os
 * endpoints autenticados do portal do paciente (escopados pela própria
 * organização do paciente, sem depender do site público estar publicado).
 * Duplicada deliberadamente em vez de generalizada: são dois contextos de
 * segurança diferentes (público/anônimo vs. autenticado por guard
 * "patient") e a busca pública é código sensível já testado — não vale o
 * risco de acoplar as duas por uma abstração comum.
 */
export function usePatientAvailabilitySearch() {
    ...
    async function loadUnits() {
        ...
        try {
            units.value = await fetchJson<AvailabilityUnit>(
                availability.units().url,
            );
        } catch {
            error.value =
        ...
        try {
            specialties.value = await fetchJson<AvailabilitySpecialty>(
                availability.specialties({
                    query: { unit_id: selectedUnitId.value },
                }).url,
            );
        } catch {
            error.value =
