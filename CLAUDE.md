...
# Gestão de Clínicas — Regras do projeto

Este arquivo é lido por Claude Code e outros agentes de IA. As regras abaixo
complementam as guidelines geradas pelo Laravel Boost acima e são
específicas deste projeto. `AGENTS.md` contém o mesmo conteúdo desta seção
para ferramentas que não leem `CLAUDE.md`.
...
- **Consulte a documentação da versão instalada** (via Boost `search-docs` ou documentação oficial) antes de usar uma API de pacote — não invente métodos, opções ou assinaturas que não existem na versão instalada.
